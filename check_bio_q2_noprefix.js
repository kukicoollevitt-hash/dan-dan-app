...
const content = fs.readFileSync('public/BRAINUP/science/bio_content.js', 'utf8');

const unitPattern = /(?<![a-z_])bio_(\d+):\s*\{/g;
let match;
const issues = [];
...
while ((match = unitPattern.exec(content)) !== null) {
  ...
  const unitKey = 'bio_' + unitNum.padStart(2, '0');
...
if (issues.length > 0) {
  ...
  console.log('✅ bio 01~20: 모든 q2 보기가 정상입니다.');
}
