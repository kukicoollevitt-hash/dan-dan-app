...
const content = fs.readFileSync('public/BRAINUP/science/earth_content.js', 'utf8');

const unitPattern = /(?<![a-z_])earth_(\d+):\s*\{/g;
let match;
const issues = [];
...
while ((match = unitPattern.exec(content)) !== null) {
  ...
  const unitKey = 'earth_' + unitNum.padStart(2, '0');
...
if (issues.length > 0) {
  ...
  console.log('✅ earth 01~20: 모든 q2 보기가 정상입니다.');
}
