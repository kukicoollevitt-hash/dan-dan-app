...
const content = fs.readFileSync('public/BRAINUP/social/on_law_content.js', 'utf8');

const unitPattern = /on_law_(\d+):\s*\{/g;
let match;
const errors = [];

while ((match = unitPattern.exec(content)) !== null) {
  const unitNum = match[1];
  const unitKey = `on_law_${unitNum}`;
...
if (errors.length === 0) {
  console.log('✅ on_law 01~20: 모든 explain 값이 passage에 포함되어 있습니다.');
} else {
