#!/usr/bin/env python3
"""
on_modern_content.js의 모든 단원 explain 값을 검증합니다.
...
# on_modern_content.js 읽기
with open('/Users/dandan/Desktop/dan-dan-app_new/public/BRAINUP/korlit/on_modern_content.js', 'r', encoding='utf-8') as f:
    content = f.read()
...
for i in range(1, 41):
    ...
    # passage 추출 - 더 유연한 패턴
    passage_pattern = rf'{unit_key}:\s*\{{[^}}]*?passage:\s*\[(.*?)\]'
    ...
    # explain 추출
    explain_pattern = rf'{unit_key}:\s*\{{.*?explain:\s*\{{([^}}]*)}}'
...
# 검증
print("=" * 70)
print("on_modern explain 검증 결과 (q1~q4)")
print("=" * 70)
...
for unit_key in sorted(units.keys()):
    ...
    for q in ['q1', 'q2', 'q3', 'q4']:
        ...
        if explain not in data['passage_plain']:
            ...
            if b_collision:
                unit_issues.append(f"  {q}: [✓] ⚠<b>태그</b> - '{explain[:40]}...'")
            else:
                pass  # OK
