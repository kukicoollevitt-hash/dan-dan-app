...
const content = fs.readFileSync('public/BRAINUP/korlit/on_modern_content.js', 'utf8');

const unitPattern = /on_modern_(\d+):\s*\{/g;
let match;
const errors = [];
...
while ((match = unitPattern.exec(content)) !== null) {
  ...
  const unitKey = `on_modern_${unitNum}`;
...
if (errors.length === 0) {
  console.log('✅ on_modern 21~40: 모든 explain 값이 passage에 포함되어 있습니다.');
} else {
