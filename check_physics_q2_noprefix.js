...
const content = fs.readFileSync('public/BRAINUP/science/physics_content.js', 'utf8');

const unitPattern = /(?<![a-z_])physics_(\d+):\s*\{/g;
let match;
const issues = [];
...
while ((match = unitPattern.exec(content)) !== null) {
  ...
  const unitKey = 'physics_' + unitNum.padStart(2, '0');
  ...
  q2Opts.forEach((opt, i) => {
    ...
    if ((i + 1) !== q2Answer && similarity > 0.4) {
      console.log('  ' + (i+1) + '번: ' + opt + ' ⚠️실제' + claimedParagraph + '문단과 일치!(' + (similarity*100).toFixed(0) + '%)');
      console.log('      → 실제 ' + claimedParagraph + '문단: ' + actualContent.substring(0, 60) + '...');
      hasIssue = true;
    } else {
      console.log('  ' + (i+1) + '번: ' + opt + isAnswer);
    }
  });
...
if (issues.length > 0) {
  ...
  console.log('✅ physics 01~20: 모든 q2 보기가 정상입니다.');
}
