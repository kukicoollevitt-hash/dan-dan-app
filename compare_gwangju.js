...
// 1. CSV 파일에서 광주 초등학교 데이터 읽기
const csvPath = './public/images/2025년도_학년별·학급별 학생수(초)_광주광역시교육청.csv';
...
console.log(`\n📊 광주 CSV에서 500명 이상 학교: ${csvSchools.length}개\n`);

// 2. school_list.html에서 현재 광주 학교 목록 추출
const htmlPath = './public/school_list.html';
const htmlContent = fs.readFileSync(htmlPath, 'utf-8');

const gwangjuRegex = /\{\s*sido:\s*"광주광역시",\s*sigungu:\s*"([^"]+)",\s*name:\s*"([^"]+)",\s*students:\s*(\d+)/g;
const existingSchools = new Set();
let match;
while ((match = gwangjuRegex.exec(htmlContent)) !== null) {
  existingSchools.add(match[2]);
}

console.log(`📋 현재 school_list.html의 광주 학교: ${existingSchools.size}개\n`);
...
if (missingSchools.length > 0) {
  ...
  for (const sigungu of sigunguOrder) {
    ...
    for (const s of schools) {
      console.log(`  { sido: "광주광역시", sigungu: "${s.sigungu}", name: "${s.name}", students: ${s.students}, competitor: "" },`);
    }
  }
}
