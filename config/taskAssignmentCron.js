...
async function executeSchedule(userProgress, schedule) {
  try {
    // 1. 선택 가능한 모든 단원 ID 생성
    const availableUnits = getAllUnitIds(schedule.fields, schedule.subjects);
    ...
    // 4. 다음 과제 선택 (순차적, 미완료, 미배정)
    const selectedUnits = selectNextUnits(
      availableUnits,
      completedUnitIds,
      assignedUnitIds,
      schedule.taskCount
    ...
    if (selectedUnits.length > 0) {
      const newTasks = selectedUnits.map(unitId => ({
        id: unitId,
        unitId: unitId,
        title: `자동 부여 과제: ${unitId}`,
        series: 'BRAIN업', // 기본값 (시리즈 정보는 unitId로부터 유추 가능)
        field: schedule.fields[0] || '분야', // 첫 번째 분야
        subject: schedule.subjects[0] || '과목', // 첫 번째 과목
        isAI: true,
        assignedAt: new Date(),
        status: 'pending',
        progress: 0
      ...
      // 마지막 AI 과제 부여 시간 업데이트
      userProgress.studyRoom.lastAIAssignedAt = new Date();

      console.log(`[크론] ${userProgress.grade} ${userProgress.name} - ${selectedUnits.length}개 과제 부여: ${selectedUnits.join(', ')}`);
    } else {
      console.log(`[크론] ${userProgress.grade} ${userProgress.name} - 부여 가능한 과제 없음 (모두 완료 또는 배정됨)`);
    }

    // 6. 스케줄의 마지막 실행 시간 업데이트
