#!/bin/bash
cd /Users/dandan/Desktop/dan-dan-app_1212

# 백업
cp public/BRAINUP/person/deep_people2_content.js public/BRAINUP/person/deep_people2_content.js.backup_final_$(date +%s)

python3 << 'PYTHON_SCRIPT'
# Unit 39, 40 전체 확장
import re

FILE = 'public/BRAINUP/person/deep_people2_content.js'
...
i = 0
while i < len(lines):
    # Unit 39 passage 시작 찾기
    if i < len(lines) - 3 and 'deep_people2_39:' in lines[i]:
        # Unit 구조 그대로 복사하면서 passage만 교체
        j = i
        while j < len(lines):
            ...
            if 'passage: [' in lines[j]:
                ...
                output.extend(new_passages)
                i = j
                break
            j += 1
            if 'vocab: [' in lines[j-1]:
                i = j
                break
        continue
