const fs = require("fs");
const content = fs.readFileSync("/Users/dandan/Desktop/brainmoon_academy0326/public/BRAINUP/social/deep_law_content.js", "utf8");

for (let i = 1; i <= 20; i++) {
  const unit = "deep_law_" + String(i).padStart(2, "0");

  const passageStart = content.indexOf(unit + ":");
  if (passageStart === -1) continue;

  const nextUnit = "deep_law_" + String(i + 1).padStart(2, "0");
