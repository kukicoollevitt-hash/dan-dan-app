...
function findDeepFiles(dir) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      results = results.concat(findDeepFiles(filePath));
    } else if (file.startsWith('deep_') && file.endsWith('.html')) {
...
const files = findDeepFiles(brainupDir);

console.log(`Found ${files.length} deep_ HTML files\n`);
