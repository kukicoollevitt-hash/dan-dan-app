...
// earth_01~10 파일의 보완학습 버튼 visibility 수정
// style="display:none;" 제거 및 onclick 수정
...
for (let i = 1; i <= 10; i++) {
  ...
  if (content.includes('class="report-btn-remedial"') && content.includes('style="display:none;"')) {
    ...
    if (content.includes(oldLine)) {
      ...
      console.log(`[OK] earth_${num} 보완학습 버튼 수정 완료`);
      modified = true;
    }
  }

  if (!modified) {
    console.log(`[SKIP] earth_${num} 이미 수정되어 있음`);
  }

  fs.writeFileSync(filePath, content, 'utf8');
}

console.log('\n✅ earth_01~10 보완학습 버튼 visibility 수정 완료!');
