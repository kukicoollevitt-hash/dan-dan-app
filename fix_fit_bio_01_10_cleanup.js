...
// fit_bio_01~10 파일에서 중복 인라인 REMEDIAL_BANK 제거 및 참조 수정
...
for (let i = 1; i <= 10; i++) {
  ...
  if (inlineRemedialPattern.test(content)) {
    ...
    console.log(`[OK] fit_bio_${num} 인라인 REMEDIAL_BANK 블록 제거`);
    modified = true;
  }
  ...
  if (content.includes(oldRef1)) {
    ...
    console.log(`[OK] fit_bio_${num} REMEDIAL_BANK[key] → window.REMEDIAL_BANK[key]`);
    modified = true;
  }
  ...
  if (content.includes(oldRef2)) {
    ...
    console.log(`[OK] fit_bio_${num} for...in REMEDIAL_BANK → window.REMEDIAL_BANK`);
    modified = true;
  }
  ...
  if (content.includes(oldVocabPattern)) {
    ...
    console.log(`[OK] fit_bio_${num} vocab-state \${unit} → window.CUR_UNIT`);
    modified = true;
  }
  ...
  if (content.includes(oldGeoPattern)) {
    ...
    console.log(`[OK] fit_bio_${num} current-geo-tab \${unit} → window.CUR_UNIT`);
    modified = true;
  }

  // savedTab 에러 수정 (fit_bio용)
  ...
  if (content.includes(oldSavedTab)) {
    ...
    console.log(`[OK] fit_bio_${num} savedTab 정의 추가`);
    modified = true;
  }
