...
for (let i = 11; i <= 20; i++) {
  ...
  if (content.includes(oldPattern1)) {
    ...
    console.log(`[OK] fit_earth_${num} 보완학습 버튼 활성화 (패턴1)`);
    modified = true;
  }
  ...
  if (content.includes(oldPattern2)) {
    ...
    console.log(`[OK] fit_earth_${num} onclick 패턴 수정`);
    modified = true;
  }
  ...
  if (oldPattern3.test(content)) {
    ...
    console.log(`[OK] fit_earth_${num} display:none 제거`);
    modified = true;
  }
  ...
  if (modified) {
    ...
    console.log(`[SKIP] fit_earth_${num} 이미 활성화되어 있거나 패턴 불일치`);
  }
}

console.log('\n✅ fit_earth_11~20 보완학습 버튼 활성화 완료!');
