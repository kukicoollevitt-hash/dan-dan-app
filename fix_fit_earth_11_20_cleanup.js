...
// fit_earth_11~20 파일에서 중복 인라인 REMEDIAL_BANK 제거 및 참조 수정
...
for (let i = 11; i <= 20; i++) {
  ...
  if (inlineRemedialPattern.test(content)) {
    ...
    console.log(`[OK] fit_earth_${num} 인라인 REMEDIAL_BANK 블록 제거`);
    modified = true;
  }
  ...
  if (content.includes(oldRef1)) {
    ...
    console.log(`[OK] fit_earth_${num} REMEDIAL_BANK[key] → window.REMEDIAL_BANK[key]`);
    modified = true;
  }
  ...
  if (content.includes(oldRef2)) {
    ...
    console.log(`[OK] fit_earth_${num} for...in REMEDIAL_BANK → window.REMEDIAL_BANK`);
    modified = true;
  }
  ...
  if (content.includes(oldVocabPattern)) {
    ...
    console.log(`[OK] fit_earth_${num} vocab-state \${unit} → window.CUR_UNIT`);
    modified = true;
  }
  ...
  if (content.includes(oldGeoPattern)) {
    ...
    console.log(`[OK] fit_earth_${num} current-geo-tab \${unit} → window.CUR_UNIT`);
    modified = true;
  }
  ...
  if (content.includes(oldSavedTab)) {
    ...
    console.log(`[OK] fit_earth_${num} savedTab 정의 추가`);
    modified = true;
  }
  ...
  if (modified) {
    ...
    console.log(`[SKIP] fit_earth_${num} 이미 정리되었거나 패턴 불일치`);
  }
}

console.log('\n✅ fit_earth_11~20 정리 완료!');
