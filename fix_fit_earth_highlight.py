#!/usr/bin/env python3
"""
fit_earth_02~20.html 파일에 하이라이트 CSS와 함수를 추가하고,
fit_earth_content.js의 explain 값을 </b> 태그 뒤 텍스트로 수정하는 스크립트
"""
...
def add_highlight_to_html(filepath):
    ...
    if '.evidence-highlight' in content:
        print(f"[{filename}] 이미 하이라이트 CSS 있음 - 스킵")
        return False

    # 1. CSS 추가 (</style> 앞에)
    # body { background: url...learning-bg.jpg 패턴 찾기
    css_pattern = r'(body\s*\{\s*background:\s*url\([^)]+learning-bg\.jpg[^}]*\}\s*)(</style>)'

    if re.search(css_pattern, content, re.DOTALL):
        content = re.sub(css_pattern, r'\1' + highlight_css + r'\n\2', content, flags=re.DOTALL)
        modified = True
        print(f"[{filename}] CSS 추가 완료")
    else:
        # 대안: </style> 바로 앞에 추가
        style_end_pattern = r'(</style>)'
        if re.search(style_end_pattern, content):
            content = re.sub(style_end_pattern, highlight_css + r'\n\1', content, count=1)
            modified = True
            print(f"[{filename}] CSS 추가 완료 (대안 패턴)")
        else:
            print(f"[{filename}] CSS 삽입 위치를 찾을 수 없음")
    ...
    if re.search(func_pattern, content):
        ...
        modified = True
        print(f"[{filename}] 함수 추가 완료")
    else:
        print(f"[{filename}] 함수 삽입 위치를 찾을 수 없음")

    # 3. gradeQuiz 끝에 highlightEvidence() 호출 추가
    # saveUnitProgressToServer 호출 후에 추가
    grade_pattern = r"(console\.log\('\[gradeQuiz\] 서버에 본문학습 채점 결과 저장 요청 완료'\);\s*\})"
    ...
    if re.search(grade_pattern, content) and 'highlightEvidence' not in content.split('gradeQuiz')[1].split('resetQuiz')[0]:
      ...
      if (typeof window.highlightEvidence === 'function') {
        ...
        modified = True
        print(f"[{filename}] gradeQuiz에 highlightEvidence 호출 추가")
    ...
    if re.search(reset_pattern, content) and 'clearEvidenceHighlights' not in content.split('resetQuiz')[1].split('saveVocabState')[0] if 'saveVocabState' in content else 'clearEvidenceHighlights' not in content.split('resetQuiz')[1][:500]:
        replacement = r"""\1

      // 하이라이트 제거
      if (typeof window.clearEvidenceHighlights === 'function') {
        window.clearEvidenceHighlights();
      }\2"""
        content = re.sub(reset_pattern, replacement, content)
        modified = True
        print(f"[{filename}] resetQuiz에 clearEvidenceHighlights 호출 추가")
  ...
  if (typeof window.highlightEvidence === 'function') {
    window.highlightEvidence();
        ...
        modified = True
        print(f"[{filename}] saveReadingProgress에 highlightEvidence 호출 추가")
...
for i in range(2, 21):
    ...
    if os.path.exists(filepath):
        print(f"\n=== {filename} 처리 중 ===")
        add_highlight_to_html(filepath)
    else:
        print(f"\n[{filename}] 파일 없음")
