const fs = require("fs");
const content = fs.readFileSync("/Users/dandan/Desktop/brainmoon_academy0326/public/BRAINUP/person/fit_people2_content.js", "utf8");

for (let i = 1; i <= 40; i++) {
  const unit = "fit_people2_" + String(i).padStart(2, "0");

  const passageStart = content.indexOf(unit + ":");
  if (passageStart === -1) continue;

  const nextUnit = "fit_people2_" + String(i + 1).padStart(2, "0");
