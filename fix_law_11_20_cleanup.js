...
for (let i = 11; i <= 20; i++) {
  ...
  // 1. 인라인 스크립트 내의 기존 window.REMEDIAL_BANK = {...} 블록 제거
  // 패턴: // ===== 보완학습 문제 뱅크 ... window.REMEDIAL_BANK = {...};
  const inlineRemedialPattern = /\/\/ ===== 보완학습 문제 뱅크 \(객관식 2문제씩\) =====\s*window\.REMEDIAL_BANK = \{[\s\S]*?\};\s*(?=\/\/|function|let|const|var|\n\s*\n)/;

  if (inlineRemedialPattern.test(content)) {
    content = content.replace(inlineRemedialPattern, '');
    console.log(`[OK] law_${num} 인라인 REMEDIAL_BANK 블록 제거`);
    modified = true;
  }
  ...
  if (content.includes(oldRef1)) {
    ...
    console.log(`[OK] law_${num} REMEDIAL_BANK[key] → window.REMEDIAL_BANK[key]`);
    modified = true;
  }
  ...
  if (content.includes(oldRef2)) {
    ...
    console.log(`[OK] law_${num} for...in REMEDIAL_BANK → window.REMEDIAL_BANK`);
    modified = true;
  }
  ...
  if (modified) {
    ...
    console.log(`[SKIP] law_${num} 이미 정리되었거나 패턴 불일치`);
  }
}

console.log('\n✅ law_11~20 REMEDIAL_BANK 정리 완료!');
