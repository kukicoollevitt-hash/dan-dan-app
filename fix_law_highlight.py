#!/usr/bin/env python3
"""
law_02~20.html 파일에 하이라이트 구조를 일괄 적용합니다.
"""
...
for i in range(2, 21):
    ...
    if 'evidence-highlight' not in content:
        ...
        if '<style>' in content:
            ...
            content = content.replace(' <style>', ' <style>\n' + css_to_add, 1)
            modified = True
            print(f"[{filename}] CSS 추가됨 (공백 있음)")
        else:
...
{m.group(2)}
    ...
    if 'function clearEvidenceHighlights' not in content:
        # </script> 전 law_content.js 로드 전에 추가
        pattern = r"(\}\s*\n</script>\s*\n+<!-- 외부 JS: 단원 콘텐츠 로드)"
        if re.search(pattern, content):
            def repl_fn(m):
