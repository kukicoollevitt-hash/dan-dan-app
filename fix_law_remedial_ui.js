...
// law_01~10 파일의 openRemedial, gradeRemedial 함수에서
// ①②③④ 동그라미 번호와 해설 추가
...
for (let i = 1; i <= 10; i++) {
  ...
  if (content.includes(oldOpenPattern1)) {
    ...
    console.log(`[OK] law_${num} openRemedial 동그라미 번호 추가`);
    modified = true;
  }

  // 2. gradeRemedial 함수에서 selected = i → selected = parseInt(inp.value) 변경
  ...
  if (content.includes(oldGradePattern1)) {
    ...
    console.log(`[OK] law_${num} gradeRemedial selected 값 수정`);
    modified = true;
  }

  // 3. gradeRemedial 함수에서 정답 해설 추가
  ...
  if (content.includes(oldCorrectPattern)) {
    ...
    console.log(`[OK] law_${num} gradeRemedial 정답 해설 추가`);
    modified = true;
  }

  // 4. gradeRemedial 함수에서 오답 해설 추가
  ...
  if (content.includes(oldWrongPattern)) {
    ...
    console.log(`[OK] law_${num} gradeRemedial 오답 해설 추가`);
    modified = true;
  }
  ...
  if (modified) {
    ...
    console.log(`[SKIP] law_${num} 이미 수정되었거나 패턴 불일치`);
  }
}

console.log('\n✅ law_01~10 보완학습 UI 수정 완료!');
