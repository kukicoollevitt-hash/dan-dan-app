#!/usr/bin/env python3
"""
on_world1_02~40.html 파일 (하드코딩된 explain 구조)의 gradeQuiz 함수에서
qXMsg 변수 선언 추가
"""
...
def process_file(file_path):
    ...
    # 이미 qXMsg 변수가 있으면 건너뛰기
    if 'const q1Msg' in content:
        print(f"  ⏭ {os.path.basename(file_path)} 이미 수정됨")
        return False

    # on_world1_02~40 구조: const explain = {...}; 다음에 추가
    # q5로 끝나는 explain 객체 찾기
    old_pattern = r"(const explain = \{[^}]*q5:[^}]+\};)"
    new_text = r"""\1

      // detail 변수 선언 (content.js의 pack.detail 사용)
      const pack = window.CONTENTS && window.CONTENTS[window.CUR_UNIT];
    ...
    if modified:
        ...
        print(f"  ⏭ {os.path.basename(file_path)} 변경 없음")
        return False

def main():
    base_path = "/Users/dandan/Desktop/dan-dan-app_1217/public/BRAINUP/worldlit"

    print("on_world1 HTML 파일 qXMsg 변수 추가 (하드코딩 explain 구조용)...")

    for i in range(2, 41):  # 02~40
