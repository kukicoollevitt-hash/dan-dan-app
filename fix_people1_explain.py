#!/usr/bin/env python3
"""
on_people1_content.js의 q2, q3 explain 값을 수정합니다.
...
filepath = '/Users/dandan/Desktop/dan-dan-app_new/public/BRAINUP/person/on_people1_content.js'

with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

def get_paragraph2_start(passages):
    """2번째 문단의 시작 텍스트 추출 (</b> 뒤 텍스트)"""
    if len(passages) < 2:
        return None
    p2 = passages[1]
    # </b> 뒤의 텍스트 찾기
...
def get_vocab_context(passages, vocab_word):
    ...
    for p in passages:
        ...
        pattern = rf'<b>{re.escape(vocab_word)}</b>([^<]{{5,50}})'
...
for i in range(15, 41):
    ...
    # 다음 유닛 시작점 찾기
    next_unit = content.find(f"on_people1_{i+1:02d}:", unit_start)
    if next_unit == -1:
    ...
    if q2_match:
        ...
        if '문단:' in current_q2:
            new_q2 = get_paragraph2_start(paragraphs)
            if new_q2:
