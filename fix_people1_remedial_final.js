...
const REMEDIAL_DATA = {
  ...
  '14': {
    literal: { q: "장수왕이 수도를 옮긴 곳은?", options: ["한양","평양","경주","부여"], answer: 1, explanation: "장수왕은 남진 정책을 위해 수도를 평양으로 옮겼습니다." },
...
for (let i = 1; i <= 20; i++) {
  ...
  // window.REMEDIAL_BANK 스크립트 생성
  const remedialScript = `<script>
  // 보완학습 데이터 (learning-common.js 로드 전에 설정)
  ...
  // 기존에 삽입된 보완학습 스크립트가 있으면 제거
  html = html.replace(/<script>\s*\/\/ 보완학습 데이터[\s\S]*?window\.REMEDIAL_BANK[\s\S]*?<\/script>\s*/g, '');

  // learning-common.js 로드 전에 삽입
  html = html.replace(
    /<script src="\/assets\/js\/learning-common\.js/,
    remedialScript + '\n  <script src="/assets/js/learning-common.js'
  );
...
console.log('\n🎉 온한국인물(1) 01~20 보완학습 최종 수정 완료!');
console.log('- window.REMEDIAL_BANK를 learning-common.js 로드 전에 설정');
