...
// physics_11~20 파일의 gradeRemedial 함수에서 value 처리 수정
...
for (let i = 11; i <= 20; i++) {
  ...
  if (oldPattern.test(content)) {
    ...
    console.log(`[OK] physics_${num} gradeRemedial value 처리 수정`);
  }
  ...
  if (!content.includes('value="${optIdx}"')) {
    ...
    if (content.includes('<input type="radio" name="${key}-q${idx}">')) {
      ...
      console.log(`[OK] physics_${num} radio value 추가`);
    }
  }
...
console.log('\n✅ physics_11~20 gradeRemedial 수정 완료!');
