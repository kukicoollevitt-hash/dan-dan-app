#!/usr/bin/env python3
"""
pol_02~20.html 파일에 하이라이트 구조를 일괄 적용합니다.
"""
...
for i in range(2, 21):
    ...
    if 'evidence-highlight' not in content:
        ...
        if '</style>' in content:
            ...
            modified = True
            print(f"[{filename}] CSS 추가됨 (style 시작)")
        else:
...
{m.group(2)}
    ...
    if 'highlightEvidence' not in content:
        ...
        pattern = r"(if \(content\) renderSolutions\(content\);)\s*\n(\s*\})\s*\n(\s*// showSubmitSuccess|\s*document\.getElementById\('save-progress-btn'\))"
        if re.search(pattern, content):
            def repl_save(m):
                return f'''{m.group(1)}
{m.group(2)}
    ...
    if 'function clearEvidenceHighlights' not in content:
        # </script> 전 pol_content.js 로드 전에 추가
        pattern = r"(</script>\s*\n+\s*<!-- 외부 JS: 단원 콘텐츠 로드)"
        if re.search(pattern, content):
            def repl_fn(m):
