#!/usr/bin/env python3
"""
세계문학(1) world1_01~40 resetQuiz 함수에 입력창 활성화 코드 추가
"""
...
def fix_resetQuiz(filepath):
    ...
    # resetQuiz 함수 패턴 찾기
    # 패턴: document.querySelectorAll('#tab-reading input[type="radio"]').forEach(r => r.checked = false);
    old_pattern = r"(function resetQuiz\(\) \{)\s*\n\s*(document\.querySelectorAll\('#tab-reading input\[type=\"radio\"\]'\)\.forEach\(r => r\.checked = false\);)"
    ...
    new_content, count1 = re.subn(old_pattern, new_code, content)

    if count1 == 0:
        return False, "패턴1 미발견"

    # q5 value 초기화 다음에 disabled = false 추가
    # 패턴: document.getElementById("q5").value = ""; 다음 줄
...
# 실행
base = "/Users/dandan/Desktop/dan-dan-app/public/BRAINUP/worldlit"

print("=== 세계문학(1) world1 resetQuiz 수정 ===\n")

for i in range(1, 41):
    filename = f"world1_{i:02d}.html"
