...
for (let i = 11; i <= 20; i++) {
  ...
  if (content.includes(oldPattern1)) {
    ...
    console.log(`[OK] soc_${num} 보완학습 버튼 활성화 (패턴1)`);
    modified = true;
  }
  ...
  if (!modified && content.includes(oldPattern2)) {
    ...
    console.log(`[OK] soc_${num} 보완학습 버튼 활성화 (패턴2)`);
    modified = true;
  }
  ...
  if (!modified && content.includes('report-btn-remedial') && content.includes('style="display:none;"')) {
    ...
    console.log(`[OK] soc_${num} 보완학습 버튼 display:none 제거`);
    modified = true;
  }

  if (!modified) {
    console.log(`[SKIP] soc_${num} 이미 활성화되어 있거나 패턴 불일치`);
  }

  fs.writeFileSync(filePath, content, 'utf8');
}

console.log('\n✅ soc_11~20 보완학습 버튼 활성화 완료!');
