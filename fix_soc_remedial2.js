...
// soc_01~10 파일의 gradeRemedial 함수에서 value 처리 수정
...
for (let i = 1; i <= 10; i++) {
  ...
  if (oldPattern.test(content)) {
    ...
    console.log(`[OK] soc_${num} gradeRemedial value 처리 수정`);
  }
  ...
  if (!content.includes('value="${optIdx}"')) {
    ...
    if (content.includes('<input type="radio" name="${key}-q${idx}">')) {
      ...
      console.log(`[OK] soc_${num} radio value 추가`);
    }
  }
