...
function findSocialDeepFiles(dir) {
  let results = [];
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      results = results.concat(findSocialDeepFiles(filePath));
    } else if ((file.startsWith('deep_geo') || file.startsWith('deep_soc') || file.startsWith('fit_geo') || file.startsWith('fit_soc')) && file.endsWith('.html')) {
...
const files = findSocialDeepFiles(brainupDir);

console.log(`Found ${files.length} social deep/fit HTML files\n`);
