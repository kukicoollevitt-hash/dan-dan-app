...
for (let i = 1; i <= 10; i++) {
  ...
  // 인라인 REMEDIAL_BANK 블록 제거 (스크립트 내부에 정의된 것)
  ...
  if (inlineRemedialPattern.test(content)) {
    ...
    console.log(`[OK] world1_${num} 인라인 REMEDIAL_BANK 블록 제거`);
    modified = true;
  }

  // REMEDIAL_BANK[key] → window.REMEDIAL_BANK[key]
  ...
  if (content.includes(oldRef1)) {
    ...
    console.log(`[OK] world1_${num} REMEDIAL_BANK[key] → window.REMEDIAL_BANK[key]`);
    modified = true;
  }

  // for...in REMEDIAL_BANK → window.REMEDIAL_BANK
  ...
  if (content.includes(oldRef2)) {
    ...
    console.log(`[OK] world1_${num} for...in REMEDIAL_BANK → window.REMEDIAL_BANK`);
    modified = true;
  }
  ...
  if (modified) {
    ...
    console.log(`[SKIP] world1_${num} 이미 정리되었거나 패턴 불일치`);
  }
}

console.log('\n✅ world1_01~10 REMEDIAL_BANK 정리 완료!');
