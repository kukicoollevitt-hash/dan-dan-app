#!/usr/bin/env python3
"""
world2_02~40.html 파일에 하이라이트 구조를 일괄 적용합니다.
"""
...
{m.group(2)}
    ...
    if 'highlightEvidence' not in content:
        ...
        patterns = [
            r"(if \(content\) renderSolutions\(content\);)\s*\n(\s*\})\s*\n([\s\S]*?)(// showSubmitSuccess)",
            r"(if \(content\) renderSolutions\(content\);)\s*\n(\s*\})\s*\n\s*\n(\s*// showSubmitSuccess)"
        ]
...
{m.group(2)}
    ...
    if 'function clearEvidenceHighlights' not in content:
        # </script> 앞에 추가 (world2_content.js 로드 전)
        patterns = [
            r'(</script>\s*\n+\s*<!-- 외부 JS: 단원 콘텐츠 로드)',
            r'(</script>\s*\n+\s*<!-- 보완학습 표시 영역)',
            r'(\}\s*\n\s*\}\s*\n</script>)\s*\n+\s*(<!-- 외부 JS)',
            r"(showSubmitSuccess\('분석리포트'\);\s*\n\s*\})\s*\n(\s*</script>)"
        ]
        ...
        for idx, pattern in enumerate(patterns):
            if re.search(pattern, content):
                if idx == 0 or idx == 1:
                    def repl_fn(m):
                        return highlight_functions + '\n</script>\n\n' + m.group(1).replace('</script>\n', '').replace('</script>', '')
                    content = re.sub(pattern, repl_fn, content, count=1)
                elif idx == 2:
                    def repl_fn2(m):
                        return m.group(1).replace('</script>', '') + highlight_functions + '\n</script>\n\n' + m.group(2)
                    content = re.sub(pattern, repl_fn2, content, count=1)
                else:
                    def repl_fn3(m):
                        return m.group(1) + highlight_functions + '\n' + m.group(2)
                    content = re.sub(pattern, repl_fn3, content, count=1)
                modified = True
                found = True
