/**
 * ✅ 단원 자동 인식 (강화)
 * 우선순위: ?unit=people2_XX → 파일명 people2_XX.html → 제목 숫자
 */
(function () {
  ...
  if (qs) {
    ...
    if (m) unit = `people2_${m[1].padStart(2, '0')}`;
  }

  if (!unit) {
    const m2 = location.pathname.toLowerCase().match(/geo[_-]?(\d{1,2})\.html/);
    if (m2) unit = `people2_${m2[1].padStart(2, '0')}`;
  }

  if (!unit && document.title) {
    const m3 = document.title.match(/(\d{1,2})/);
    if (m3) unit = `people2_${m3[1].padStart(2, '0')}`;
  }

  window.CUR_UNIT = unit || 'people2_01';
})();
