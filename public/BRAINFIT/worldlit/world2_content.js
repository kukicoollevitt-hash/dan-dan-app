/**
 * ✅ 단원 자동 인식 (강화)
 * 우선순위: ?unit=world2_XX → 파일명 world2_XX.html → 제목 숫자
 */
(function () {
...
(function () {
  ...
  /* ✅ 특정 학생 + 단원 기준 진행도 key 만들기
     → dan-progress:학년_이름_전화:world2_04 */
  ...
  function readDoneList() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      const unit = k.slice(prefix.length);  // world2_01, world2_02 ...
  ...
  const ProgressManager = {
    ...
    markComplete() {
      ...
      const unit = detectUnit();                  // world2_04
      const key  = buildProgressKey(stu, unit);   // dan-progress:학생키:world2_04
    ...
    /* ✅ 현재 학생의 완료 단원 리스트 얻기 (world2_01, world2_03 ...) */
    getStudentProgress() {
  ...
  try {
    ...
    if (data.ok && Array.isArray(data.completedUnits)) {
      ...
      data.completedUnits.forEach(unit => {
        const key = keyPrefix + unit;  // "dan-progress:학생키:world2_01"
...
window.sendLearningLog = async function () {
  try {
    const unit = window.CUR_UNIT || 'world2_02';
