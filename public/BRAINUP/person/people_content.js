/**
 * ✅ 단원 자동 인식 (강화)
 * 우선순위: ?unit=people_XX → 파일명 people_XX.html → 제목 숫자
 */
(function () {
  ...
  if (qs) {
    const m = qs.toLowerCase().match(/people[_-]?(\d{1,2})/);
    if (m) unit = `people_${m[1].padStart(2, '0')}`;
  }

  if (!unit) {
    const m2 = location.pathname.toLowerCase().match(/people[_-]?(\d{1,2})\.html/);
    if (m2) unit = `people_${m2[1].padStart(2, '0')}`;
  }

  if (!unit && document.title) {
    const m3 = document.title.match(/(\d{1,2})/);
    if (m3) unit = `people_${m3[1].padStart(2, '0')}`;
  }

  window.CUR_UNIT = unit || 'people_01';
})();
...
window.CONTENTS = Object.assign(window.CONTENTS, {
  ...
  /* ===== people_06 : "인물를 보고 길을 찾아가다" (샘플 데이터) ===== */
  people_06: {
    labelNo: '06',
  ...
  /* ===== people_07 : "세계 여러 나라의 위치와 특징" (샘플 데이터) ===== */
  people_07: {
    labelNo: '07',
  ...
  /* ===== people_08 : "우리나라의 인물적 특성 이해하기" (샘플 데이터) ===== */
  people_08: {
    labelNo: '08',
  ...
  /* ===== people_10 : "환경 문제와 지속 가능한 발전" (샘플 데이터) ===== */
  people_10: {
    labelNo: '10',
  ...
  /* ===== people_11 ~ people_20 샘플 데이터 (더 간략하게) ===== */
...
function getCurrentUnit() {
  return (window.CUR_UNIT || 'people_01');
}

window.renderVocabFill = function () {
  const unit = window.CUR_UNIT || 'people_01';
...
window.gradeQuiz = function () {
  const pack = window.CONTENTS[window.CUR_UNIT] || window.CONTENTS.people_01;
...
(function () {
  ...
  /* ✅ 단원 감지: people_01, people_02 ... */
  function detectUnit() {
    let unitParam = new URLSearchParams(location.search).get('unit');
    if (!unitParam) {
      const m = location.pathname.match(/people_(\d+)\.html/i);
      if (m) unitParam = `people_${m[1].padStart(2, '0')}`;
    }
    return (unitParam || (window.CUR_UNIT || 'people_01')).toLowerCase();
  }
  ...
  /* ✅ 특정 학생 + 단원 기준 진행도 key 만들기
     → dan-progress:학년_이름_전화:people_04 */
  ...
  function readDoneList() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      const unit = k.slice(prefix.length);  // people_01, people_02 ...
  ...
  const ProgressManager = {
    ...
    markComplete() {
      ...
      const unit = detectUnit();                  // people_04
      const key  = buildProgressKey(stu, unit);   // dan-progress:학생키:people_04
    ...
    /* ✅ 현재 학생의 완료 단원 리스트 얻기 (people_01, people_03 ...) */
    getStudentProgress() {
  ...
  try {
    ...
    if (data.ok && Array.isArray(data.completedUnits)) {
      ...
      data.completedUnits.forEach(unit => {
        const key = keyPrefix + unit;  // "dan-progress:학생키:people_01"
...
window.sendLearningLog = async function () {
  try {
    const unit = window.CUR_UNIT || 'people_02';
