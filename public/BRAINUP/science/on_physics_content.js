/**
 * ✅ 단원 자동 인식 (강화) - BRAINON (on_physics_XX)
 * 우선순위: ?unit=on_physics_XX → 파일명 on_physics_XX.html → 제목 숫자
 * ⚠️ HTML에서 이미 CUR_UNIT 설정했으면 덮어쓰지 않음
 */
(function () {
  // 이미 설정된 CUR_UNIT이 있고, on_physics_로 시작하면 덮어쓰지 않음
  ...
  if (qs) {
    const m = qs.toLowerCase().match(/on_physics[_-]?(\d{1,2})/);
    if (m) unit = `on_physics_${m[1].padStart(2, '0')}`;
  }

  if (!unit) {
    const m2 = location.pathname.toLowerCase().match(/on_physics[_-]?(\d{1,2})\.html/);
...
window.CONTENTS = Object.assign(window.CONTENTS, {
  /* ===== on_physics_01 : "힘이 세면 뭐든지 바꿀 수 있을까?" ===== */
  on_physics_01: {
    labelNo: '01',
  ...
  /* ===== on_physics_03 : "지구와 달에서 다른 몸무게의 비밀" ===== */
  on_physics_03: {
    labelNo: '03',
  ...
  /* ===== on_physics_07 : "세계 여러 나라의 위치와 특징" (샘플 데이터) ===== */
  on_physics_07: {
    labelNo: '07',
  ...
  /* ===== on_physics_11 ~ on_physics_20 ===== */

  /* ===== on_physics_11 : "빛의 성질과 반사" ===== */
  on_physics_11: {
    labelNo: '11',
    title: '💡 거울 속 세상, 빛의 반사',
    passage: [
...
function getCurrentUnit() {
  return (window.CUR_UNIT || 'on_physics_01');
}
...
window.renderVocabFill = function () {
  ...
  const unit = window.CUR_UNIT || 'on_physics_01';
...
window.gradeQuiz = function () {
  const pack = window.CONTENTS[window.CUR_UNIT] || window.CONTENTS.on_physics_01;
...
(function () {
  ...
  /* ✅ 단원 감지: on_physics_01, on_physics_02 ... */
  function detectUnit() {
    ...
    return (unitParam || (window.CUR_UNIT || 'on_physics_01')).toLowerCase();
  }
  ...
  /* ✅ 특정 학생 + 단원 기준 진행도 key 만들기
     → dan-progress:학년_이름_전화:on_physics_04 */
  ...
  function readDoneList() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      const unit = k.slice(prefix.length);  // on_physics_01, on_physics_02 ...
  ...
  const ProgressManager = {
    ...
    markComplete() {
      ...
      const unit = detectUnit();                  // on_physics_04
      const key  = buildProgressKey(stu, unit);   // dan-progress:학생키:on_physics_04
    ...
    /* ✅ 현재 학생의 완료 단원 리스트 얻기 (on_physics_01, on_physics_03 ...) */
    getStudentProgress() {
  ...
  try {
    ...
    if (data.ok && Array.isArray(data.completedUnits)) {
      ...
      data.completedUnits.forEach(unit => {
        const key = keyPrefix + unit;  // "dan-progress:학생키:on_physics_01"
...
window.sendLearningLog = async function () {
  try {
    const unit = window.CUR_UNIT || 'on_physics_02';
